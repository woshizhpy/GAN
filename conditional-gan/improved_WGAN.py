...
import data_utils
import logging
logging.basicConfig(filename='output_train.log', filemode='a',level=logging.INFO)
...
class Improved_WGAN(object):
	...
	def train(self):
		...
		logging.info("Start training WGAN...")
		...
		for t in range(self.FLAGS.iter):
			...
			if current_step % self.FLAGS.display_every == 0:
				...
				logging.info("Epoch {}, Current_step {}".format(self.data.epoch, current_step))
				logging.info("Discriminator loss :{}".format(d_cost))
				logging.info("Generator loss     :{}".format(g_cost))
				logging.info("---------------------------------")
