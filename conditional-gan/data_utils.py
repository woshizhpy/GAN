...
class Data(object):
    def __init__(self, img_feat, tags_idx, a_tags_idx, test_tags_idx, z_dim, vocab_processor):
        ...
        self.hair_idx = vocab_processor._mapping['type1']
        self.eyes_idx = vocab_processor._mapping['type2']
        self.gen_info()
...
def load_train_data(data_img_dir, tag_path, prepro_dir, vocab_path, shuffle_time=1):
    ...
    # read tags from pokeType file
    # currently we use two types as conditions and type1, type2, HP, Attack, Defence as labels
    with open(tag_path) as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
            c_tags = []
            k_tags = {}
            row = row[0].encode('utf-8').decode('utf-8-sig').strip().split('\t')
            Id = row[0]
            Name = row[1]
            type_1 = row[2]
            type_2 = row[3]
            k_tags['HP'] = int(row[4])
            k_tags['Attack'] = int(row[5])
            k_tags['Defence'] = int(row[6])
            attrib = {'type1':type_1, 'type2':type_2}
            

            a_tags = ' '.join([attrib['type1'], attrib['type2']])
            c_tags = [attrib['type1'] + ' type1', attrib['type2'] + ' type2']
            k_tags[c_tags[0]] = float('Inf')
            k_tags[c_tags[1]] = float('Inf') 
            ...
            items = ['a', 'b']
            for item in items:
                img_dir = os.path.join(data_img_dir, type_1)
                img_path = os.path.join(img_dir, '{}_{}.png'.format(Id, item))
                if not os.path.isfile(img_path):
                    continue

                feat = misc.imread(img_path, mode="RGBA")[:, :, :3]
    ...
    print("after {}".format(vocab_processor._mapping))
...
#     max_length = max([len(tags.split()) for tags in raw_tags])
#     vocab_processor = VocabularyProcessor(max_document_length=max_length, vocabulary=vocabulary)
#     print('raw_tags : {}'.format(raw_tags))
#     print('attrib_tags : {}'.format(attrib_tags))
#     print("before {}".format(vocab_processor._mapping))
#     tags_idx = vocab_processor.transform(raw_tags)
#     a_tags_idx = vocab_processor.transform(attrib_tags, 2)
#     print("after {}".format(vocab_processor._mapping))
#     print("max sentence length: {}".format(max_length))
